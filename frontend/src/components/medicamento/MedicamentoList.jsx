          ...
          {filtrados.map(med => {
            ...
            return (
              <div key={med.id} className="card-produto" onClick={() => setMedSelecionado(med)}>
                ...
                {!vitrine && (
                  <div style={{ margin: '0.2rem 0 0.5rem', fontSize: '0.88rem', color: 'var(--texto-suave)', display: 'flex', flexDirection: 'column', gap: '0.2rem', backgroundColor: 'var(--bg)', padding: '0.6rem', borderRadius: 'var(--raio-sm)' }}>
                    <span>📦 <strong>Estoque:</strong> <strong style={{ color: med.estoque < 5 ? 'var(--vermelho)' : 'var(--azul)' }}>{med.estoque}</strong> un.</span>
                    <span>📅 <strong>Validade:</strong> {formatarData(med.date)}</span>
                  </div>
                )}
